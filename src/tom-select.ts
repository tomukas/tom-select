...
export default class TomSelect extends MicroPlugin(MicroEvent) {
	...
	setup() {
		...
		addEvent(
			...
			(e) => {
				...
				if (target_match)
					self.onOptionHover(e as MouseEvent, target_match);
			},
			{ capture: true }
		);
		...
		addEvent(focus_node, "keydown", (e) =>
			self.onKeyDown(e as KeyboardEvent)
		);
		...
		addEvent(control_input, "input", (e) =>
			self.onInput(e as KeyboardEvent)
		);
	...
	setActiveOption(option: null | HTMLElement, scroll: boolean = true): void {
		if (option === this.activeOption || (option && !option.dataset.value)) {
			return;
		}
	...
	refreshOptions(triggerDropdown: boolean = true) {
		...
		if (same_query) {
			...
			if (active_option) {
				active_group = active_option.closest(
					"[data-group]"
				) as HTMLElement;
			}
		}
		...
		for (i = 0; i < n; i++) {
			...
			for (j = 0, k = optgroups && optgroups.length; j < k; j++) {
				...
				const [group_order_i, group_fragment] = getGroupFragment(
					optgroup,
					order
				);
